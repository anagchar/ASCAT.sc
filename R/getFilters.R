                       ...
                       projectname="")
{
  ...
  try({
    ...
    amb_mask <- if (exclude_ambiguous) !ambiguous else rep(TRUE, length(ambiguous))
    keep <- qualities<=thresholdQual & nrecords>=thresholdNrec & amb_mask
    keep2 <- !(qualities<thresholdQual & nrecords<thresholdNrec)
    keep2 <- keep2 & amb_mask
